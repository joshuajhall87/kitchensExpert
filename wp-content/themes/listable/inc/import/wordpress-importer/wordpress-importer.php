 ...
 */
if ( class_exists( 'WP_Importer' ) ) {
	class WPGrade_WP_Import extends WP_Importer {
		...
		function process_term_meta( $term, $term_id ) {

			if ( ! function_exists( 'update_term_meta' ) ) {
				return;
			}

			if ( isset( $term['term_meta'] ) && ! empty( $term['term_meta'] ) ) {
				$meta = $term['term_meta'];

				if ( is_array( $meta ) ) { // in case of WXR_Parser_Regex
					if ( isset( $meta['meta_key' ] ) && isset( $meta['meta_value'] ) && ! empty( $meta['meta_key']) ){
						$success = update_term_meta( $term_id, $meta['meta_key' ], $meta['meta_value' ] );
					}
				} elseif ( is_object( $meta ) ) { // in case of WXR_Parser
					if ( isset( $meta->meta_key ) && isset( $meta->meta_value ) && ! empty( $meta->meta_key ) ){
						$success = update_term_meta( $term_id, (string) $meta->meta_key, (string) $meta->meta_value );
					}
				}
			}
		...
		function process_menu_item( $item ) {
			...
			if ( $id && ! is_wp_error( $id ) ) {
				...
				$item['postmeta'] = apply_filters( 'wp_import_post_meta', $item['postmeta'], $id, $item );
		...
		function remap_featured_images() {
			// cycle through posts that have a featured-classic image
