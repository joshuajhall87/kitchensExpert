<?php
/**
 * Plugin Name: WP Job Manager - Stats
 * Plugin URI: https://astoundify.com/downloads/wp-job-manager-stats/
...
function wpjms_init(){
	...
	/* Settings */
	if( is_admin() ) require_once( WPJMS_PATH . 'includes/class-wpjms-settings.php' );
	...
	/* Admin Stats */
	if( is_admin() ) require_once( WPJMS_PATH . 'includes/class-wpjms-stats-admin.php' );
	...
	if( class_exists( 'WooCommerce' ) ){
		...
		/* WC Advanced Paid Listing */
		elseif( function_exists( 'jwapl_init' ) ){
			require_once( WPJMS_PATH . 'includes/wc-advanced-paid-listings/wc-advanced-paid-listings.php' );
		}
	}
}
...
function wpjms_updater(){
	require_once( WPJMS_PATH . 'library/astoundify-pluginupdater/PluginUpdater.php' );
