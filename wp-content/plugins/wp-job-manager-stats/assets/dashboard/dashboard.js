jQuery( document ).ready( function( $ ){
	...
	function refresh_chart(){
		...
		var data = {
			action    : 'wpjms_job_dashboard_chart',
			nonce     : wpjms.ajax_nonce,
			date_from : date_from,
			date_to   : date_to,
			stat_id   : stat_id,
		...
		$.post( wpjms.ajax_url, data, function( ajax_response ) {
			wrap.height( wrap.height() + 'px' );
			$( '#wpjms_job_dashboard_chart' ).html( $( ajax_response ).html() );
