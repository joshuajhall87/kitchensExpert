...
class WPJMS_Setup{
	...
	public function my_job_actions( $actions, $job ) {
		$actions['stats'] = array( 'label' => __( 'View stats', 'wp-job-manager-stats' ), 'nonce' => true );
		return $actions;
	}
	...
	public function job_manager_job_dashboard_column_total_views( $job ) {
		...
		/*  Get Stats Data */
		$stats_data = new WPJMS_Stats_Data( array( 'post_ids' => array( $post_id ), 'stat_ids'  => 'visits' ) );
		$stat_total = $stats_data->get_all_stats();

		echo isset( $stat_total[ 'visits' ] ) ? $stat_total[ 'visits' ] : 0;
	}
	...
	public function scripts(){
		...
		wp_register_script( 'date-range-picker', WPJMS_URL . 'assets/date-range-picker/jquery.daterangepicker.js', array( 'jquery', 'moment-js' ), '0.6.0', true );
		$date_range_picker_lang = array(
			...
			'default-range'    => __( 'Please select a date range between %d and %d days', 'wp-job-manager-stats' ),
