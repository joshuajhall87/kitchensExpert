...
class WPJMEL_Map_Setup{
	...
	public function __construct() {
		...
		/* Remove Fields */
		add_filter( 'job_manager_job_listing_data_fields', array( $this, 'remove_job_listing_mb_field' ) );
		add_filter( 'resume_manager_resume_fields', array( $this, 'remove_resume_mb_field' ) );
	...
	public function scripts() {
		...
		if ( $input && $page_ids && is_page( $page_ids ) ){
			...
			if ( $item_id ){
				...
				/* Lock status */
				$lock_status = get_post_meta( $item_id, 'geolocation_map_lock_status', true );
				$options['lock'] = 'lock' == $lock_status ? $lock_status : $options['lock'];
			}
