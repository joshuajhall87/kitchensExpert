...
class WP_Job_Manager_Extended_Location{
	...
	public function __construct() {
		...
		if( class_exists( 'WP_Job_Manager' ) ){
			...
			$map_key = get_option( 'wpjmel_google_maps_api_key' );
	...
	public function updater() {
		require_once $this->path . 'includes/updater/class-astoundify-updater.php';
		new Astoundify_Updater_Extended_Location( $this->file );
	}
	...
	public function register_scripts() {
		...
		/* Add Google Map Key */
		$key = strip_tags( get_option( 'wpjmel_google_maps_api_key' ) );
	...
	public function get_user_ip_location( $ip = false ) {
		...
		$hash = 'wpjmel_location_' . md5( $ip . $this->version );
		...
		if ( ! $location_data = get_transient( $hash ) ) {
			...
			$location_data['formatted_address'] = implode( $loc, ', ' );
