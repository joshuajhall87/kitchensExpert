 ...
 *
 * @since 1.1.9
 * @commit fY9L3vOqu+L5aEJyK7sI6QEE
 *
 */
class WP_Job_Manager_Field_Editor_Fields extends WP_Job_Manager_Field_Editor {

	public static $always_required  = array( 'job_title', 'candidate_name', 'job_description' );
	...
	function get_all_fields( $field_group = NULL, $force_update = FALSE ) {
		...
		foreach( $this->wpjm_fields as $wpjm_field_group ) {
			$this->fields[ $wpjm_field_group ] = $this->get_group_fields( $wpjm_field_group );
		}

		if ( $this->wprm_active() ) foreach( $this->wprm_fields as $wprm_field_group ) {
			$this->fields[ $wprm_field_group ] = $this->get_group_fields( $wprm_field_group );
		}
	 ...
	 * @return mixed
	 */
	function get_group_fields( $field_group ) {
		...
		$this->fields[ $field_group ] = $this->merge_with_custom_fields( $this->get_default_fields( $field_group ), $field_group );
	...
	function get_fields( $field_group = NULL, $filter = 'all', $fields_with_children = TRUE ) {
		...
		if ( ! $field_group ) {
			...
			foreach( $this->wpjm_fields as $wpjm_field_group ) {
				$af[ $wpjm_field_group ] = $this->get_fields( $wpjm_field_group, $filter, $fields_with_children );
			}

			if ( $this->wprm_active() ) foreach( $this->wprm_fields as $wprm_field_group ) {
				$af[ $wprm_field_group ] = $this->get_fields( $wprm_field_group, $filter, $fields_with_children );
			}
		...
		switch ( $filter ) {
			...
			case 'default':
				$fields = $this->get_customized_fields( $field_group );
				break;

			case 'customized':
				$fields = $this->fields_list_filter( $this->get_customized_fields( $field_group ), array('origin' => 'default') );
				break;

			case 'custom':
				$fields = $this->fields_list_filter( $this->get_custom_fields( $field_group ), array('origin' => 'custom') );
				break;

			case 'disabled':
				$fields = $this->fields_list_filter( $this->get_all_fields( $field_group ), array('status' => 'disabled') );
				break;

			case 'enabled':
				$fields = $this->fields_list_filter( $this->get_all_fields( $field_group ), array('status' => 'disabled'), 'NOT' );
				break;
			case 'all':
			default:
	...
	function get_default_fields( $field_group = NULL, $force_update = false ) {
		...
		if ( in_array( $field_group, $this->wpjm_fields ) ) {
			...
			// And by clearing value set in the instance (for Jobs)
			$this->integration()->wpjm()->remove_traces();
		}
		...
		if ( $this->wprm_active() && in_array( $field_group, $this->wprm_fields ) ) {
			...
			// And by clearing value set in the instance (for Resumes)
			$this->integration()->wprm()->remove_traces();
		}
	...
	function clear_all_fields( $force_update = true ) {
		...
		if( $force_update ) do_action( 'job_manager_field_editor_flush_cache_from_ajax' );
	...
	function merge_with_custom_fields( $default_fields, $field_group = null ) {
		...
		$custom_fields  = $this->get_custom_fields( $field_group );
