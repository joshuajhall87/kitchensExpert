...
class WP_Job_Manager_Field_Editor_Fields_Options extends WP_Job_Manager_Field_Editor_Fields {
	...
	public static function maybe_get_label( $value, $args ){
		...
		/**
		 * Exit if not enabled in settings, or $value is empty.  Also check that $args is an array that isn't empty
		 */
		if ( empty( $output_label ) || empty( $value ) || ! is_array( $args ) || empty( $args ) ) {
			return $value;
		}

		$option_label_types = apply_filters( 'field_editor_get_custom_field_option_label_types', array( 'select', 'multiselect', 'radio' ), $value, $args );
		 ...
		 */
		$translated_value = WP_Job_Manager_Field_Editor_Translations::translate( $args['options'][ $value ], $args['meta_key'], 'options', $args['field_group'] );
