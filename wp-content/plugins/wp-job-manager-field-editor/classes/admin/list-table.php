...
Class WP_Job_Manager_Field_Editor_List_Table extends WP_List_Table {
	...
	function output_item_data_values( $item ) {
		...
		$no_data_output = apply_filters( 'job_manager_field_editor_listtable_no_data_output', array( 'fields', '_meta_key', 'field_group', 'required' ), $item );
		...
		if ( 'enable' === $current_action ) {
			...
			foreach( $process_fields as $post_id ) {
				...
				if ( ! is_numeric( $post_id ) ) {
					...
					$post_id = $cpt->insert_field_post( $post_id, FALSE );
				}
